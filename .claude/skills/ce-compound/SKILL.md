...
argument-hint: "[optional: brief context] [mode:headless|mode:lightweight] "
---
...
/ce-compound mode:lightweight           # Force the single-pass lightweight run
```
...
If invoked specifically to create or bootstrap `CONCEPTS.md` from scratch rather than to document a solved problem, do not run the normal phases — `ce-compound` populates `CONCEPTS.md` only as a side effect of documenting a real learning (it seeds the *learning's area*, not the whole repo; see Phase 2.4). Repo-wide concept-map creation is out of scope for this skill; this harness has no automated refresh skill. Ask the user to run `ce-compound` against a concrete solved problem instead, then exit.

## Mode Detection

Check `$ARGUMENTS` for a `mode:headless` or `mode:lightweight` token. Tokens starting with `mode:` are flags, not context — strip any `mode:*` token from arguments before treating the remainder as the brief context hint.
...
| **Lightweight (forced)** | `mode:lightweight` in arguments | Skip the auto Full-vs-Lightweight decision and run **Lightweight Mode** (single-pass, no subagents) regardless of context budget. Still interactive for the Discoverability Check — but lightweight only emits a tip, it never edits instruction files. |
...
- `scripts/session-history/` — session discovery and extraction scripts bundled inside this skill so session-history support is self-contained
...
- **Related Docs Finder** → `/tmp/compound-engineering/ce-compound/{run_id}/related.json` (links, stale-doc candidates, overlap assessment)
...
#### 3. **Related Docs Finder**
   ...
   - Writes to `related.json`: Links, relationships, stale-doc candidates, and overlap assessment (score + which dimensions matched). Returns only the artifact path.
...
2. **Check the overlap assessment** from the Related Docs Finder before deciding what to write:
   ...
   | **Moderate** — same problem area but different angle, root cause, or solution | **Create the new doc** normally. Flag the overlap for the Phase 2.5 stale-doc note. |
...
### Phase 2.5: Stale-Doc Note (manual review)

After writing the new learning, note — but do not act on — whether it makes any older doc stale. If the Related Docs Finder surfaced a doc whose approach this fix now contradicts, supersedes, renders overly broad, or overlaps enough to warrant consolidation, name it in the completion output as a **stale-doc candidate for manual review**.

This harness has no automated refresh skill. Do not edit other docs here — a human decides whether and how to revise them. Surfacing the candidate is a courtesy pointer, not a step that mutates the tree.

Always capture the new learning first. Flagging a stale neighbor is optional and never a prerequisite for documentation.

### Discoverability Check

After the learning is written and the stale-doc note is made, check whether the project's instruction files would lead an agent to discover and search `docs/solutions/` before starting work in a documented area. This runs every time — the knowledge store only compounds value when agents can find it.
...
- Any code-heavy issue → preserve code simplification as a **read-only documentation review**. Inspect the solution draft's code examples and explanatory claims inline, or dispatch a generic subagent seeded with a local prompt only to return suggestions. Do not mutate product code from this phase unless the user explicitly asks for a separate code-simplification pass.
...
In lightweight mode, the overlap check is skipped (no Related Docs Finder subagent). This means lightweight mode may create a doc that overlaps with an existing one. That is acceptable — note any obvious overlap in the output as a manual-review candidate and move on. Do not attempt a broad consolidation from a lightweight session.
...
Stale-doc candidates: <none | docs flagged for manual review>
