...
<blockquote class="markdown-callout markdown-callout-warning">
  <p class="callout-title">바인딩을 열려면 변수를 비운다</p>
  <p>주소를 적어 넣어야 열린다고 생각하기 쉽지만 반대다. 모든 인터페이스에 게시하려면 <code>SEARXNG_HOST</code>를 주석 처리한 채로 두고, 방화벽에서 포트를 허용한다. <code>127.0.0.1</code>처럼 특정 주소를 적으면 그 주소로만 게시된다.</p>
</blockquote>
