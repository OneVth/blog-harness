...
| 금지 | 예 | 수준 | 이유 |
|---|---|---|---|
| **카테고리 동명 태그** | `Embedded`, `DSA`, `Projects` | ERROR | 카테고리는 도메인, 태그는 횡단 조회. 역할이 겹친다 |
| **복수형** | `Arrays` ✗ → `Array` | **WARN** | URL 분열. **아래 참조** |
| **주관적 태그** | `추천`, `꿀팁`, `핵심` | ERROR | 검색어가 아니다 |
| **개인 분류 태그** | `기억할것`, `공부`, `type/생성` | ERROR | Obsidian용이지 블로그용이 아니다 |
| **강의 내 실습 주제** | `고추건조기` | — | 실습마다 태그가 생긴다. Source 태그로 묶이니 충분하다. **기계로 못 잡는다 — 사람이 판단** |
| **`삽질`** | (deprecated) | ERROR | `트러블슈팅`으로 통일 |
...
```
Redis · Kubernetes · HTTPS · macOS · DNS · Windows · AWS · iOS · CSS
```

복수형 판정은 **휴리스틱이라 오탐이 난다.** ERROR로 막으면 `Redis` 태그 하나에
발행이 차단되고, 그러면 린터를 끄게 된다.
...
| **promote** | 고유명이다. 린터의 허용 목록에 등록한다. 다음부터 조용해진다 |

상세: `RULES.md` POST-05a
