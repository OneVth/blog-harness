...
>
> **블록 수식(`$$...$$`)은 렌더된다** (검증됨 2026-07). 인라인 `$...$` 도 렌더된다.
> **단, 한 수식 안에 밑줄 `_` 이 2개 이상이면 깨진다** — Tistory 마크다운이 `_..._` 를
> 기울임으로 짝지어 먹어 수식에 `<em>` 이 박히고, KaTeX 가 파싱을 못 한다. 실측:
> `\underbrace{...}_{n+1} \times \underbrace{...}_{O(1)}` 는 안 뜨고, `\max_i`(단일 `_`)는 뜬다.
>
> **그래서 아래첨자 2개 이상은 평문으로 푼다** (`\underbrace{}_{}` → `(상태 수) × (전이 비용)`,
> `\max_i dp[i]` → `\max(dp)`). 백슬래시-공백 `\ ` 도 피한다. `lint_post` POST-14 가 잡는다.
